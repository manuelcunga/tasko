...
export class CreateuserProviderUsecase {
  ...
  async execute(data: CreateUserInput): Promise<UserOutput> {
    ...
    const checkNifExists = await this.userRepo.findByNif(data.nif);

    if (checkNifExists) {
      throw new BadRequestException(messages.nifAlreadyExists);
    }

    const checkPhoneExists = await this.userRepo.findByPhone(data.phone);
